...
import com.mazanex.profile.config.GlitchTipErrorReporter;
...
public class TestGlitchTipController {
    ...
    public String testLog() {
        logger.info("Verificación de logs ordinarios: Este mensaje debería aparecer en GlitchTip Logs de ms-profile.");
        return "Log de profile enviado correctamente a la cola.";
    }

    // 2. Probar envío de advertencia explícita (Message)
    @GetMapping("/message")
    public String testMessage() {
        errorReporter.captureMessage("Alerta manual: Intento sospechoso detectado en ms-profile", SentryLevel.WARNING);
        return "Mensaje de advertencia reportado en profile.";
    }

    // 3. Probar captura de excepciones inesperadas (Genera un Issue)
    @GetMapping("/error")
    public String testError() {
        throw new RuntimeException("Simulación de fallo crítico en el servicio de perfiles.");
    }
}