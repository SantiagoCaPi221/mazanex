...
export default function Page() {
  ...
  const update = async (e: any) => {
    e.preventDefault();
    const old = localStorage.getItem(`old_pass_${user.id}`);

    if (pass.new === pass.current || pass.new === old) {
      return setMsg({
        text: "Ya utilizaste esta contraseña anteriormente.",
        type: "error",
      });
    }

    const res = await authService.actualizarPassword(user.id, {
      ...user,
      password: pass.new,
    });
    ...
    if (res) {
      setUser(res);
      localStorage.setItem(`old_pass_${user.id}`, pass.current);
    ...
    <div className="max-w-lg mx-auto bg-white p-8 rounded-3xl border border-slate-100 shadow-sm space-y-6">
      ...
      <form onSubmit={update} className="space-y-4">
        ...
        <input
          ...
          className="w-full p-4 bg-slate-50 rounded-2xl outline-none focus:ring-2 focus:ring-indigo-100 border border-transparent focus:border-indigo-500 font-bold text-slate-700"
        />
        <button className="w-full py-4 bg-indigo-600 text-white font-black rounded-2xl hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100">
